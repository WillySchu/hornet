...
def test_type_from_name_array():
    ...
    expected = semantic.Type(kind=semantic.TypeKind.ARRAY, element_type=semantic.Type(kind=semantic.TypeKind.INT), size=5)
...
def test_type_from_name_slice_nested():
    type_expr = parser.SliceTypeExpr(element_type=parser.SliceTypeExpr(element_type=parser.SliceTypeExpr(element_type='bool')))
    expected = semantic.Type(
